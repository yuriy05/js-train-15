...
function generateRandomPassword(length) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    result += characters[charactersIndex];
  }
  return result;
}
...
function calculateCircleArea(radius) {
  ...
  if (typeof radius === "number") {
    ...
    console.error("Помилка! Обрахунок неможливий");
    return null;
  }
}
...
function roundObjectValues(obj) {
  ...
  newArray = newArray.map(([key, value]) => {
...
function calculateVolumeCylinder(radius, height) {
  ...
  let volumeCylinder = Math.ceil(Math.PI * Math.pow(radius, 2) * height);
...
function isTotalPriceExceedsMaxPrice(products, maxPrice) {
  ...
  let totalPrice = products.reduce((accumulator, products) => accumulator + products.price, 0);
  let totalValue = Math.fround(totalPrice);
  let maxValue = Math.fround(maxPrice);

  let compare = totalValue > maxValue;
  return compare;
}
